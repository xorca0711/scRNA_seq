...
def main():
    import numpy as np
    import pandas as pd
    out=PAPER/'trials/u5_ipf_compatibility';out.mkdir(exist_ok=True)
    ...
    try:
        ...
        for cohort in ['GSE136831','GSE135893']:
            subprocess.run([str(rscript),str(PAPER/'trials/u5_compatibility_normalize.R'),cohort],cwd=ROOT,check=True)
            df=pd.read_csv(PAPER/'cache/u5_ipf_compatibility'/(cohort+'_components.csv.gz'))
        ...
        pd.DataFrame(donor_rows).to_csv(PAPER/'cache/u5_ipf_compatibility/donor_scores.csv.gz',index=False)
