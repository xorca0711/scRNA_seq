...
genes <- readLines(file.path(paper,'trials/w1_ipf_compatibility/component_genes.txt'))
...
dir.create(file.path(paper,'cache/w1_ipf'),showWarnings=FALSE,recursive=TRUE)
write.csv(do.call(rbind,output),gzfile(file.path(paper,'cache/w1_ipf',paste0(cohort,'_components.csv.gz'))),row.names=FALSE)
write.csv(do.call(rbind,norm),file.path(paper,'trials/w1_ipf_compatibility',paste0(cohort,'_normalization.csv')),row.names=FALSE)
capture.output(sessionInfo(),file=file.path(paper,'cache/w1_ipf',paste0(cohort,'_R_session.txt')))
