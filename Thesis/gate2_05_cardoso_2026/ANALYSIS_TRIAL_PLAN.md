...
| D0 | 0 | What does the Choi 2020 deposit contain, the origin of the DATP state the C series leans on? (ran 2026-09-13 under roadmap paper 2; relocated here 2026-09-15) | run; Descriptive only | [`trials/d0_data_reality_check/d0_summary.md`](trials/d0_data_reality_check/d0_summary.md) |
...
| [D0](#d0-the-choi-2020-deposit-checked-as-the-origin-of-the-datp-state-run-2026-09-13-relocated-here-2026-09-15) | the Choi 2020 deposit | one library per condition again; relocated from the withdrawn paper-2 folder |
...
## D0. The Choi 2020 deposit, checked as the origin of the DATP state (run 2026-09-13, relocated here 2026-09-15)

This trial ran on 2026-09-13 under the roadmap's paper-2 folder and was moved
here on 2026-09-15 at the owner's instruction (DEVELOPMENT decision 21). The
paper-2 folder was withdrawn until the owner has read the paper; the deposit
check stays, as an extension of this folder, because the state it
characterises, the DATP of Choi et al. 2020, is the one the C-series rows lean
on. The trial keeps its D0 identifier so that its run record, its artefact
names and claims rows C58 to C64 stay true, and nothing in it was re-run. Its
marker extract moved with it:
[`choi_2020_extracts.json`](choi_2020_extracts.json), read from the Europe
PMC XML because the PMC web rendering strips italicised gene symbols. The Gate
1 trial that the withdrawn plan proposed (state recovery and the AT2 to DATP to
AT1 ordering) is not planned here; it belongs to the owner's future paper-2
folder.
