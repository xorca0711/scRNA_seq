...
These are figures generated by this repository's HLCA-motivated trials,
separate from the source-paper results discussed in the study note below.
Human reference mapping uses GSE178360; mouse QC trials use GSE262927.
[Trial methods, decisions and tables](ANALYSIS_TRIAL_PLAN.md).

![Transferred HLCA labels and label-transfer uncertainty in the human cohort](trials/s2_reference_mapping/s2_umap_transfer_and_uncertainty.png)

*S2: labels and uncertainty on the mapped human cohort. Labels exceeding the
uncertainty cutoff are marked unknown. Low uncertainty does not validate an
identity missing from the reference; the trial documents this failure mode.*

![Cluster-by-type HLCA marker-score heatmap](trials/s3_hlca_marker_annotation/s3_cluster_by_type_heatmap.png)

*S3: mean own-marker scores by human cluster, displaying assigned types.
Scores expose annotation overlap and disagreement; they are neither posterior
probabilities nor evidence of cell fate.*

<details>
<summary>Donor uncertainty, annotation comparison and mouse QC figures</summary>

![Label-transfer uncertainty distributions by human donor](trials/s2_reference_mapping/s2_uncertainty_hist_per_donor.png)

*S2: donor-level distributions of cell-label uncertainty. Cells contribute to
these distributions but are not independent donor replicates.*

![Human UMAP comparing HLCA marker annotation with initial proposals](trials/s3_hlca_marker_annotation/s3_umap_hlca_vs_blind.png)

*S3: annotation schemes projected onto the same data. Their agreement is a
robustness check, not independent validation of every marker-based label.*

![Mouse cluster 23 quality-control distributions](trials/s4_cluster23_qc/s4_qc_boxplots.png)

*S4: QC evidence underlying the low-count, ambient-like interpretation of
cluster 23. This diagnostic does not measure the amount or source of ambient
RNA directly.*

![Mouse cluster 5 subclusters compared with deposited labels](trials/s5_cluster5_subclusters/s5_umap_subclusters_vs_labels.png)

*S5: subclustering and deposited-label comparisons distinguish mixed
annotations from a single inferred identity. Resolution and author-label
