...
with observations, rivals, tests, decision limits and readiness. No four-question
limit applies. A12-S1 is an enabling source-identity question; A14 has two
separable hypotheses. Read `docs/RQ_MEASUREMENT_CONTRACTS.md` for reusable
measurement checks and the ID crosswalk; `analysis/figures/rq/README.md` owns
curated captions and the gallery. The root narrative is authored.

The rewrite grants no new hypothesis a historical claim grade. Scientific
results, thresholds and numerical records were preserved. No biological model
or embedding was refit. A3 alone was redrawn from cached coordinates to describe
sampling after infection, without asserting recovery or same-cell persistence.
See `docs/migrations/2026-09-25-rq-reframing/README.md` for provenance and checks.
Branch: `codex/rewrite-research-questions`. Next scientific work is the bounded
A10 outcome/preparation join and compatible A1 direct-mark input gate, not another
unrestricted score/rank sweep. This documentation turn did not execute them.

Script 16's old root-caption writer and obsolete A2 renderer were removed.
Use explicit `--figures` and `--replot` for cache-only rendering into a fresh
`analysis/figures/rq/renders/<run>/`; missing/invalid caches fail closed.
`--rebuild-embeddings` is explicit and uses fresh caches. Never overwrite original
run records or promote a new figure without checking its source tables/captions.
...
pitfalls_for_ai_assistants:
  ...
  - "RESEARCH_QUESTIONS.md is authored; analysis/figures/rq/README.md owns curated captions. Script 16 owns A1/A3/A4/A5 assets and generated panel facts, never root edits; script 18 owns current A2/A6–A9. Renders go into fresh run directories. Depth panels remain supporting diagnostics; A1 promoter panel g must be read with its depth and zero-detection controls (C120/C127), not cited alone as evidence for silenced-but-open chromatin. See MC6 for ownership and provenance."
...
reproduce:
  ...
  - python analysis/scripts/16_research_question_figures.py --figures A3 --replot   # explicit subset, existing caches only; fresh render directory, no authored-document or historical-run writes
