...
def check_research_layout(root: Path, result) -> None:
    ...
    roadmap = json.loads((root / 'Thesis/ROADMAP.json').read_text(encoding='utf-8'))
    for item in roadmap['papers']:
        folder = item.get('folder')
        if folder:
            result.require(not folder.startswith('Thesis/') and not folder.endswith('/'),
                           f'paper {item["order"]}: folder must be Thesis-relative')
            result.require((root / 'Thesis' / folder).is_dir(), f'missing paper folder: {folder}')
    ...
    result.equal(digest(root / entrypoint), render['code']['entrypoint_sha256'],
                 'current figure rendering code matches run identity')
