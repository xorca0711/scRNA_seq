...
def main():
    ...
    ax.legend(handles=[Line2D([],[],marker='D',color='black',ls='',label='Primary: 10 mice'),Line2D([],[],marker='o',color='#377E96',ls='',label='Each omission: 9 mice'),Line2D([],[],marker='s',color='#CA8C2C',ls='',label='S061: 3 vs 3 mice')],frameon=False,loc='lower left',fontsize=8)
    ...
    fig.tight_layout(rect=(0,.085,1,.95));save(fig,'a1_ire1_stability')
    ...
    (BASE/'reports/second_batch_summary_run.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
