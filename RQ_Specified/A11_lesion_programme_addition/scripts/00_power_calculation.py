...
def main() -> None:
    ...
    d = values[(values.module == 'HPCS_without_ADI_or_operational_markers') & (values.config == 'unc20_pooled')
               & (values.label == 'AT2') & (values.cell_floor == 50) & (values.case == 'LUAD')
               & (values.reference == 'normal')].difference.to_numpy()
    if len(d) != 23:
        raise SystemExit(f'Expected 23 discovery patients, found {len(d)}')
    dz = d.mean() / d.std(ddof=1)

    rng = np.random.default_rng(SEED)
    rows = []
    for label, effect in [('discovery effect', dz), ('half the discovery effect', dz / 2)]:
        hits = 0
        ...
        rows.append({'scenario': label, 'standardized_effect': round(float(effect), 4), 'n_patients': N_PATIENTS,
                     'wilcoxon_exact_power': round(hits / SIMULATIONS, 4), 'paired_t_power': round(float(t_power), 4)})
    ...
    record = {
        ...
        'discovery': {'n': int(len(d)), 'mean': round(float(d.mean()), 4), 'sd': round(float(d.std(ddof=1)), 4),
