...
from analysis.lib.provenance import sha256_file, code_identity, write_json_atomic, archive_existing_record
...
def main():
    ...
    for record_name in ['render_run_record.json', 'render_validation.json']:
        archive_existing_record(out / record_name)
    inputs = []
    outputs = []
    validations = []

    def read(rel):
        path = out / Path(rel).name if rel.startswith('trials/u7_proposal_figures/') else PAPER / rel
        inputs.append(dict(path=path.relative_to(ROOT).as_posix(), sha256=sha256_file(path)))
    ...
    def export(fig, stem):
        for ext in ['png', 'svg']:
            ...
            outputs.append(dict(path=path.relative_to(ROOT).as_posix(), sha256=sha256_file(path)))
        plt.close(fig)
    ...
    fig.suptitle('A12-S1 | Where does source uncertainty enter the niche analysis?', x=.035, ha='left', fontsize=16, y=.97)
    ...
    export(fig, 'rq_a12_source_context')
    ...
    fig.suptitle('A11 | Which epithelial programs accompany lesion context?', x=.035, ha='left', y=.97, fontsize=16)
    ...
    export(fig, 'rq_a11_epithelial_programs')
    ...
    fig.suptitle('A12a | Separate ligand, recipient and regulatory RNA components', x=.035, ha='left', y=.97, fontsize=16)
    ...
    export(fig, 'rq_a12_source_recipient_components')
    ...
    fig.suptitle('A12b | Recipient enrichment depends on the correlation model', x=.035, ha='left', y=.97, fontsize=16)
    ...
    export(fig, 'rq_a12_recipient_enrichment')
    ...
    fig.suptitle('A12c | Is IL1B a leading candidate for recipient target changes?', x=.035, ha='left', y=.97, fontsize=16)
    ...
    export(fig, 'rq_a12_ligand_target_candidates')
    ...
    ax.set_ylim(0, 5)
    ax.axis('off')
    fig.suptitle('A14 | Test recovery after IL-1beta withdrawal', x=.035, ha='left', y=.95, fontsize=17)
    ...
    note(fig, 'A14: does recovery after withdrawal depend on exposure history and fibroblast IL-1 reception?\n'
    ...
    export(fig, 'rq_a14_withdrawal_design')
