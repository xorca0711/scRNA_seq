...
| [M1](m1_closed_or_merely_silenced.py) | Is the AT2 identity programme closed or merely silenced in the transitional state | **NOT COMPUTABLE**, two rule defects disclosed. But its rule R2 **refuted the GSE247130 suffix map**: Cebpa is higher in the suffix the GEO order calls the knockout in all three files (14.5x, 10.9x, 4.5x), so the deposited order is inverted. Cldn4 and Sox9 agree. **Three of the figures this index first carried were wrong and the Sox9 axis was never computed; see [M4](m4_the_corroboration_the_register_claimed.py) and row C151** |
...
| [M4](m4_the_corroboration_the_register_claimed.py) | Compute the suffix corroboration the register had been quoting, after an audit found part of it was never measured | **One of the two corroborating axes did not exist.** Sox9 was absent from M1's frozen panel and dropped silently by a guard; recomputed it is 7.1x, 2.8x and 2.5x. Three further figures were wrong. The suffix map is unaffected |
