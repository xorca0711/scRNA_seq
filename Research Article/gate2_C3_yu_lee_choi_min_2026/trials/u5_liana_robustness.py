...
def main():
    ...
    if (out/'run_record.json').exists() and not resume:raise RuntimeError('Existing run; inspect and resume explicitly')
    if resume:assert json.loads((out/'run_record.json').read_text())['status']=='failed'
    ...
    if resume:
        old=json.loads((out/'specification.json').read_text())
        assert old['input_sha256']==spec['input_sha256'] and old['configs']==spec['configs'] and old['labels']==spec['labels']
    else:write_json_atomic(out/'specification.json',spec)
        ...
        for config in CONFIGS:
            ...
            for donor,groups in good.groupby('donor',sort=True,observed=True):
                ...
                for resource in ['consensus','cellchatdb']:
                    ...
                    saved=cache/f"{config['name']}_{resource}_{donor}.csv"
                    if resume and saved.exists():z=pd.read_csv(saved)
                    else:
                        try:
                            ...
                            # LIANA 1.10.0 indexes an empty mat_max array if no edge
                            # passes. Independently prove that case; never suppress
                            # another exception or fabricate zero-valued edges.
                            if 'index 0 is out of bounds' not in str(exc):raise
                            props={g:pd.Series(np.asarray((part.X[np.flatnonzero(part.obs.celltype.eq(g))]>0).mean(axis=0)).ravel(),index=part.var_names) for g in groups.celltype}
                            possible=[]
                            for pair in pairs.itertuples(index=False):
                                for edge in table.itertuples(index=False):
                                    vals=[props[pair.source].get(g,-1) for g in edge.ligand.split('_')]+[props[pair.target].get(g,-1) for g in edge.receptor.split('_')]
                                    if min(vals)>=config['expr']:possible.append((pair.source,pair.target,edge.ligand,edge.receptor))
                            assert not possible,'Native failure despite independently eligible edges'
                            z=pd.DataFrame(columns=['source','target','ligand_complex','receptor_complex','ligand_props','receptor_props','ligand_trimean','receptor_trimean','mat_max','lr_probs'])
