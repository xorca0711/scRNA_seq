...
def main():
    ...
    record=out/'liana_run_record.json'
    if record.exists():assert '--retry' in sys.argv and json.loads(record.read_text())['status']=='failed'
    ...
    try:
        for gsm in sorted(annotations.gsm.unique()):
            ...
            for config in CONFIGS:
                ...
                # Integer detections divided once avoid sparse.mean reciprocal
                # accumulation placing an exact 10% fraction just below 0.1.
                props={}
                for label in good.celltype:
                    idx=np.flatnonzero(z.obs.celltype.eq(label))
                    props[label]=pd.Series(np.asarray((z.X[idx]>0).sum(axis=0)).ravel()/len(idx),index=z.var_names)
