...
from analysis.lib.repository_paths import resolve_repo_path
...
def main():
    ...
    for item in spec['input_sources']:
        if sha256_file(resolve_repo_path(ROOT,item['path']))!=item['sha256']: raise ValueError('Frozen input changed: '+item['path'])
