...
def main():
    ...
    reference=ROOT/'Research Article/gate1_04_sikkema_2023_hlca/trials/s2_reference_mapping/reference';model_path=PAPER/'cache/u5_human_pilot/query_model'
    ...
    record=out/'processing_run_record.json';resume='--resume' in sys.argv
    if record.exists():assert resume and json.loads(record.read_text())['status'] in ['failed','interrupted_for_bounded_memory_fix']
    ...
    previous=json.loads(record.read_text()) if resume else None
    if resume:
        old=json.loads((out/'processing_specification.json').read_text());assert old['model_sha256']==spec['model_sha256'] and old['count_store_code_sha256']==spec['count_store_code_sha256']
    else:write_json_atomic(out/'processing_specification.json',spec)
    completed=previous['completed_files'] if resume else []
    state=dict(status='running',pid=os.getpid(),started_utc=spec['frozen_utc'],stage='load_reference',code=code_identity(ROOT,__file__),completed_files=completed,resume_previous_record='.history/before_manager_memory_fix.json' if resume else None);write_json_atomic(record,state);start=time.monotonic();model=None
    all_units=pd.read_csv(out/'library_subtype_units.csv').to_dict('records') if resume else []
    quality=pd.read_csv(out/'library_qc_and_annotation.csv').to_dict('records') if resume else []
    assert [r['gsm'] for r in quality]==completed
    try:
        ...
        for job in planned:
            if job['gsm'] in completed:continue
            ...
            # scvi-tools otherwise retains each query AnnData through its
            # manager registry. Keep only the model-bound reference manager.
